...
<head>
    ...
    <title>Data Pendaftaran | Sistem Informasi Klinik</title>
        ...
        <div id="page-content-wrapper">
            <nav class="navbar navbar-expand-lg navbar-light bg-white py-3 px-4 shadow-sm">
                <div class="d-flex align-items-center">
                    ...
                    <h5 class="ms-3 mb-0 d-none d-md-block">Data Pendaftaran</h5>
                </div>
            </nav>
            ...
            <div class="container-fluid px-4 mt-4">
                ...
                <div class="card mb-4 border-0 shadow-sm">
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                        <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-list-alt me-2"></i>Antrian
                            Pendaftaran</h6>
                        <a href="pendaftaran_tambah.php" class="btn btn-success btn-sm shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover align-middle" id="dataTable" width="100%">
                                ...
                                <tbody>
                                    <?php 
                                    $query = "SELECT a.*, b.nm_pasien, c.nm_paket 
                                              FROM tb_pendaftaran a
                                              JOIN tb_pasien b ON a.id_pasien = b.id_pasien
                                              JOIN tb_paket c ON a.id_paket = c.id_paket
                                              ORDER BY a.tgl_pendaftaran DESC, a.kd_pendaftaran DESC";
                                    $ambil = $koneksi->query($query);
                                    
                                    while ($pecah = $ambil->fetch_assoc()) { 
                                    ?>
                                    <tr>
                                        <td class="fw-bold text-primary"><?= $pecah['kd_pendaftaran']; ?></td>
                                        <td><?= $pecah['nm_pasien']; ?></td>
                                        <td><?= $pecah['nm_paket']; ?></td>
                                        <td><?= date('d/m/Y', strtotime($pecah['tgl_pendaftaran'])); ?></td>

                                        <td class="text-center">
                                            <?php if ($pecah['status'] == 0) { ?>
                                            <span class="badge bg-primary rounded-pill">Belum Diperiksa</span>
                                        ...
                                        <td class="text-center">
                                            <?php if ($pecah['status'] == 0) { ?>
                                            <a href="pendaftaran_batal.php?id_pendaftaran=<?= $pecah['id_pendaftaran']; ?>"
    ...
    $(document).ready(function() {
        $('#dataTable').DataTable({
            "order": [
                [3, "desc"],
                [0, "desc"]
            ] // Urutkan berdasarkan tanggal & kode terbaru
        });
    });
    </script>
</body>
